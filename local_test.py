...
def process_excel_file(file_path):
    ...
    try:
        ...
        for sheet_name in xl.sheet_names:
            ...
            # 打印工作表的前10行数据
            print(f"工作表 {sheet_name} 的前10行数据:")
...
def main():
    ...
    # 合并数据
    merged_df = merge_all_data(test_folder)
    ...
    # 输出表格保存地址
    output_folder = "D:\\test-information\\example"
    # 创建输出文件夹（如果不存在）
    ...
    print(f"\n总共处理了 {len(merged_df)} 条记录")

def create_test_excel(folder_path):
    """创建测试Excel文件"""
    # 测试数据1
    data1 = {
        'CaseID': ['TC001', 'TC002', 'TC003'],
        '需求ID Requirement ID': ['REQ001', 'REQ002, REQ003', 'REQ004'],
        '计划否Plan or not': ['是', '否', '是'],
        '测试结果Test Results': ['通过', '失败', '通过']
    }
    
    # 测试数据2
    data2 = {
        'CaseID': ['TC004', 'TC005'],
        '需求ID Requirement ID': ['REQ005', 'REQ006, REQ007, REQ008'],
        '计划否Plan or not': ['是', '是'],
        '测试结果Test Results': ['通过', '通过']
    }
    
    # 创建Excel文件
    df1 = pd.DataFrame(data1)
    df1.to_excel(os.path.join(folder_path, "测试文件1.xlsx"), index=False)
    
    df2 = pd.DataFrame(data2)
    df2.to_excel(os.path.join(folder_path, "测试文件2.xlsx"), index=False)
